...
static void ApplicationFunctionSet_SmartRobotCarMotionControl(SmartRobotCarMotionControl direction, uint8_t is_speed)
{
  ...
  case /* constant-expression */
      Forward:
    /* code */
    if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode)//Original condition was "Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode"
    {
...
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(void)
{
  ...
  if (c == '}') //Data frame tail check
  {
    ...
    else if (!error) //Check if the deserialization is successful
    {
      ...
      //Protocol is {N:101, D1:1} to switch to line tracking mode
      switch (control_mode_N)
      {
      case 1: /*<Command：N 1> motor control mode */
        Application_SmartRobotCarxxx0.Functional_Mode = TraceBased_mode;//Application_SmartRobotCarxxx0.Functional_Mode = CMD_MotorControl;
